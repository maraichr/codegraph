# Lattice: Semantic Codebase Indexing & Lineage Engine
...
The following tables are the authoritative version reference for all Lattice dependencies, finalized during Phase 1 bootstrap.
...
Consumer group: lattice-workers
...
```
Go Worker → spawns → dotnet lattice-analyzer --project {path} --output json
...
The MCP (Model Context Protocol) server exposes Lattice's capabilities as tools that LLMs running on AWS Bedrock AgentCore can invoke. This enables natural language research queries against the codebase.
...
The MCP server runs as a separate Go binary (`lattice-mcp`) deployed as a Kubernetes Deployment, registered as a tool provider in Bedrock AgentCore configuration.
...
tool_provider:
  ...
  endpoint:
    ...
    url: "https://lattice-mcp.internal.company.com/mcp/v1"
    auth:
...
api:
  ...
  config:
    log_level: info
    cors_origins: ["https://lattice.company.com"]
    jwt_issuer: "https://keycloak.company.com/realms/lattice"
    jwt_audience: "lattice-api"
...
worker:
  ...
  config:
    ...
    parse_timeout: "5m"
    max_file_size: "50MB"
    dotnet_analyzer_image: "registry.company.com/lattice-dotnet-analyzer:1.0.0"
...
```
# Ingestion metrics
lattice_index_runs_total{trigger, status}
lattice_index_run_duration_seconds{source_type, stage}
lattice_files_parsed_total{language, status}
lattice_symbols_extracted_total{kind, language}
lattice_parse_errors_total{language, error_type}
lattice_queue_depth{queue}
lattice_queue_processing_time_seconds{queue}

# API metrics
lattice_api_requests_total{method, path, status}
lattice_api_request_duration_seconds{method, path}
lattice_api_request_size_bytes{method, path}

# MCP metrics
lattice_mcp_tool_invocations_total{tool, project}
lattice_mcp_tool_duration_seconds{tool}
lattice_mcp_tool_result_size_bytes{tool}
lattice_mcp_errors_total{tool, error_type}

# Storage metrics
lattice_symbols_total{project, kind}
lattice_edges_total{project, relationship}
lattice_pg_connections_active
lattice_neo4j_queries_total{type}
lattice_neo4j_query_duration_seconds{type}
lattice_embedding_requests_total{status}
lattice_embedding_latency_seconds

# System metrics
lattice_worker_utilization_ratio
lattice_git_clone_duration_seconds
lattice_s3_download_bytes_total
```
...
- alert: IndexRunFailureRate
  expr: rate(lattice_index_runs_total{status="FAILED"}[1h]) / rate(lattice_index_runs_total[1h]) > 0.1
  for: 10m

- alert: QueueBacklog
  expr: lattice_queue_depth{queue="ingest"} > 100
  for: 30m
...
- alert: HighParseErrorRate
  expr: rate(lattice_parse_errors_total[1h]) / rate(lattice_files_parsed_total[1h]) > 0.05
  for: 30m

- alert: SlowEmbedding
  expr: histogram_quantile(0.99, lattice_embedding_latency_seconds) > 10
  for: 15m

- alert: PATExpiryWarning
  expr: lattice_gitlab_pat_expiry_days < 14
  for: 1h
```
...
│   └── client/           — Go client library for Lattice API
├── tools/
│   └── dotnet-analyzer/  — .NET Roslyn sidecar project
│       ├── Lattice.Analyzer.sln
│       ├── src/
│       │   ├── Program.cs
│       │   ├── RoslynAnalyzer.cs
│       │   └── OutputFormatter.cs
